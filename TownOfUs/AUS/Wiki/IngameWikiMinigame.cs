...
public sealed class IngameWikiMinigame(nint cppPtr) : Minigame(cppPtr)
{
    ...
    private void LoadDetailScreen()
    {
        ...
        DetailDescription.Value.text = (_selectedItem != null) ? 
            (_selectedItem.GetAdvancedDescription().ApplyKeywords() + "\nAttributes:\n" + _selectedItem.GetPassives().ApplyKeywords()) : 
            _selectedSoftItem!.GetAdvancedDescription;
        DetailDescription.Value.fontSizeMax = 2.4f;

        if (_selectedItem is ICustomAURole customRole)
        {
            DetailScreenItemName.Value.text =
                $"{customRole.RoleName}\n<size=60%>{customRole.RoleColor.ToTextColor()}{customRole.Alignment.ToDisplayString()}</size></color>";
        ...
        if (_selectedItem != null)
        {
            foreach (var ability in _selectedItem.Abilities)
            {
                ...
                text.text =
                    $"<font=\"LiberationSans SDF\" material=\"LiberationSans SDF - Chat Message Masked\">{ability.name}</font>".ApplyKeywords();
                desc.text =
                    $"<font=\"LiberationSans SDF\" material=\"LiberationSans SDF - Chat Message Masked\">{ability.description}</font>".ApplyKeywords();
