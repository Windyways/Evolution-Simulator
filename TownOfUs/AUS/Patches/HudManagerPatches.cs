...
#pragma warning restore S1854 // Unused assignments should be removed
                    ...
                    revealMods.Any(x => x.Visible && x.RevealRole))
                {
                    ...
                    {
                        if (player.TryGetModifier<DeepfakeRole>(out var fakeRole2) && fakeRole2.foolingPlayer == PlayerControl.LocalPlayer && fakeRole2.HideDeathReason)
                        {
                            var deathReason = $"<size=50%>({deathMod.DeathColor.ToTextColor()}Cleaned By The Maid</color>)</size>\n";
