...
public class SeenKill : BaseModifier
{
    ...
    public static SeenKill GetAll()
    {
        ...
        foreach (var modifier in ModifierUtils.GetActiveModifiers<SeenKill>(x => !x.Player.HasDied() &&
            !x.killer.HasDied() && !x.Player.HasModifier<ThreatenedModifier>() &&
            !(x.killer.Is(Faction.Mafia) && x.killer.HasModifier<ThreatenedModifier>()))) modifiers.Add(modifier);
    ...
    public static int GetSuspicion(PlayerControl p)
    {
        ...
        if (p.Is(Faction.Village) && p.HasModifier<GlobalReveal>()) suspicion -= 200;
...
public static class SeenKillEvents
{
    ...
    public static void EjectionEvent(EjectionEvent @event)
    {
        ...
        if (exiled != null)
        {
            ...
            if (player.TryGetModifier<SeenKill>(out var seenKill) && player.Is(Faction.Village)) 
