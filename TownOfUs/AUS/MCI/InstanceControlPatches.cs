...
public static class InstanceControlPatches
{
    ...
    public static void PostOnSwitchPlayer(PlayerControl oldBot, PlayerControl newBot)
    {
        ...
        SheriffNightChat.UpdateSheriffChat();
    }
    ...
    {
        if (player.Data.Role is Kamikaze kamikaze) Coroutines.Start(kamikaze.GenButtons(0.1f));
    }

    public static void ClearDayButtons(PlayerControl player, bool includeNecroPassing = true)
    {
        // --- KAMIKAZE ---
        if (player.Data.Role is Kamikaze kamikaze)
        {
            if (player.AmOwner)
            {
                kamikaze.meetingMenu.HideButtons();
            }
        }
    }
